...
public class Main extends JFrame {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(index == 0) {		// 회원관리
					...
					if (check == JOptionPane.YES_OPTION) {
						...
						if (result == -1) {
							JOptionPane.showMessageDialog(frame, "ER5:탈퇴처리를 할 수 없습니다.", "오류", JOptionPane.ERROR_MESSAGE);
						} else if (result == 0) {
							JOptionPane.showMessageDialog(frame, "ER6:탈퇴처리를 할 수 없습니다.", "오류", JOptionPane.ERROR_MESSAGE);
						} else {
					...
					if (check == JOptionPane.YES_OPTION) {
						...
						result = dao.delete(id);
						if (result == -1) {
							JOptionPane.showMessageDialog(frame, "ER31:데이터를 삭제할 수 없습니다.", "오류", JOptionPane.ERROR_MESSAGE);
						} else if (result== 0) {
							JOptionPane.showMessageDialog(frame, "ER32:데이터를 삭제할 수 없습니다.\\n존재하지 않는 데이터일 수 있습니다.", "오류", JOptionPane.ERROR_MESSAGE);
						} else {
							JOptionPane.showMessageDialog(frame, "데이터 삭제가 완료되었습니다.", "완료", JOptionPane.INFORMATION_MESSAGE);
	...
	private void init() {
		...
		btnIns.setBounds(440, 30, 70, 40);
		bottomPanel.add(btnIns);

		btnUpd = new JButton("수정");
		btnUpd.setBounds(520, 30, 70, 40);
